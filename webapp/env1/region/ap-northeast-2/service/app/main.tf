module "app_tier_asg" {
  ...
  sg                = ["${aws_security_group.app_sg.id}"]
  subnets           = ["${data.aws_subnet.app_private.*.id}"]
...
resource "aws_autoscaling_attachment" "app_tier_attach" {
  autoscaling_group_name = "${element(module.app_tier_asg.asg_id,0)}"
  elb                    = "${element(module.app_tier_elb.lb_id},0)"
}
