terraform {
  backend "s3" {
    ...
    key            = "terraform/webapp/env1/region/ap-northeast-2/service/app/terraform.tfstate"
...
data "aws_subnet" "app_private" {
  count = "${length(data.terraform_remote_state.network.app_private_subnets)}"
  id    = "${element(data.terraform_remote_state.network.app_private_subnets, count.index)}"
}
